...
def load_dataset(path):
    ...
    # print("Dataframe shape: {}".format(url_data.shape))
    return url_data
...
def get_train_test_set_manual(input_data, mal_start_index, mal_end_index, ben_start_index, ben_end_index):
    ...
    # print("Malicious Shape: ** {}".format(df_malicious.shape))
    # print("Benign Shape: ****** {}".format(df_benign.shape))
    total_mal = df_malicious.shape[0]
    total_ben = df_benign.shape[0]

    testset = pd.concat([df_malicious.iloc[mal_start_index:mal_end_index, :], df_benign.iloc[ben_start_index:ben_end_index, :]])
    trainset = pd.concat([df_malicious.iloc[mal_end_index+1:total_mal, :], df_benign.iloc[ben_end_index+1:total_ben, :]])
...
def save_classifier_to_disk(classifier):
...
def get_rf_classifier(features, labels):
...
def get_dt_classifier(input_features, labels):
    """
...
def get_accuracy_on_testset(classifier, test_x, y_test):
    ...
    predicted = get_verdict(classifier, test_x)

    get_n_print_classifier_accuracy(predicted, y_test)
    get_n_print_classifier_confusion_matrix(predicted, y_test)
    get_n_print_classifier_metrics(predicted, y_test)


def get_verdict(classifier, test_set_temp):
    predicted = classifier.predict(test_set_temp)
    return predicted


def get_n_print_classifier_accuracy(predicted, y_test):
    """ Get the accuracy of the model on given predicted and true labels"""
    print("\n  Accuracy = {}".format(np.mean(predicted == y_test)))


def get_n_print_classifier_confusion_matrix(predicted, y_test):
...
def get_n_print_classifier_metrics(predicted, y_test):
...
if __name__ == '__main__':
    ...
    dt_classifier = get_dt_classifier(trainX, trainY)  # type: object
